...
func (h *Handler) GetGoldGymAreaGin(c *gin.Context) {
	var (
		result   interface{}
		metadata interface{}
		err      error
	...
	span := h.tracer.StartSpan("GetGoldGymArea", ext.RPCServerOption(spanCtx))
	defer span.Finish()
	...
	case "listarea":
		...
		result, err = h.goldgymSvcArea.GetAreaByOutlet(ctx, userID, c.Query("outcode"))
	}
