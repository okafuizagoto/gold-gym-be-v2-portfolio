...
func (h *Handler) GetGoldGymCustomerGin(c *gin.Context) {
	var (
		result   interface{}
		metadata interface{}
		err      error
	...
	case "getallcustomer":
		...
		result, metadata, err = h.goldgymSvcCustomer.GetCustomer(ctx, userID, c.Query("name"), c.Query("code"), limit, offset)
	}
