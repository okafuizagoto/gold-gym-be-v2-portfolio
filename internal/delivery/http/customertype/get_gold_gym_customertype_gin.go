...
func (h *Handler) GetGoldGymCustomerTypeGin(c *gin.Context) {
	var (
		result   interface{}
		metadata interface{}
		err      error
	...
	types := c.Query("type")
	switch types {
	case "getallcustomertype":
		userID := c.GetInt("user")
		limit, _ := strconv.Atoi(c.Query("page"))
		offset, _ := strconv.Atoi(c.Query("length"))
		result, metadata, err = h.goldgymSvcCustomerType.GetCustomerType(ctx, userID, c.Query("name"), c.Query("code"), limit, offset)
	}
