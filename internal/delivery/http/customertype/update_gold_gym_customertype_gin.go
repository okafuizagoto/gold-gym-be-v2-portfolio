...
func (h *Handler) UpdateGoldGymCustomerTypeGin(c *gin.Context) {
	var (
		result         interface{}
		metadata       interface{}
		err            error
		...
		types          string
		updatecustomer goldCustomerTypeEntity.UpdateCustomerTypeData
	)
	...
	case "updatecustomertype":
		...
		updatecustomer.UpdateData.CtGoldID = userID
		result, err = h.goldgymSvcCustomerType.UpdateCustomerType(ctx, updatecustomer.UpdateData)
		if err != nil {
			log.Println("ERR", err)
		}
	}
