...
func (h *Handler) UpdateGoldGymOutletGin(c *gin.Context) {
	var (
		result       interface{}
		metadata     interface{}
		err          error
		...
		types        string
		updateoutlet goldOutletEntity.UpdateOutletData
	)
	...
	case "updateoutlet":
		...
		if err := c.ShouldBindJSON(&updateoutlet); err != nil {
			c.JSON(400, gin.H{"error": err.Error()})
			return
		}
		updateoutlet.UpdateData.OutletGoldID = userID
		err = h.goldgymSvcOutlet.UpdateOutlet(ctx, updateoutlet.UpdateData)
		if err != nil {
			log.Println("err", err)
		}
	}
